...
Usage
-----
    # Generate judge prompts from one or more result directories:
    python judge.py results/run1 results/run2 [--out results/judge_01] [--seed 42]

    # After filling judgment_template.json with scores:
    python judge.py --summary results/judge_01

    .venv/bin/python judge.py --run \
...
def cmd_run(
    ...
    for i in range(n_runs):
        ...
        if run_dir and run_dir.is_dir():
            ...
            print("[judge] Could not determine run dir from bench.py output", file=sys.stderr)
...
def cmd_auto_judge(
    ...
    for task in TEST_CASES:
        ...
        for label in labels:
            ...
            clamped = [
                min(max(int(v), 0), r["points"])
                for v, r in zip(cs, rubric)
            ]
...
def cmd_generate(result_dirs: list[Path], out_dir: Path, seed: int) -> None:
    ...
    (out_dir / "blinding_map.json").write_text(json.dumps({
        "seed": seed,
    ...
    print(f"[judge] responses:  {total_resp} total  ({total_resp // len(found_ids) if found_ids else 0} per task)")
    print(f"[judge] seed:       {seed}")
    print(f"[judge] out dir:    {out_dir}")
    print()
    print("Next steps:")
    print(f"  1. python judge.py --judge {out_dir} --judge-model <model> --judge-base-url <url>")
...
def cmd_summary(judge_dir: Path) -> None:
    ...
    blinding_map: dict = bm_data["map"]

    judgment: dict = {}
    # condition → {score, max, responses}
    cond_agg: dict = {}

    col_w = {"task": 6, "fn": 36, "run": 24, "cond": 8, "score": 8}
    header = (
        f"{'Task':<{col_w['task']}}  {'Function':<{col_w['fn']}}  "
        f"{'Run':<{col_w['run']}}  {'Cond':<{col_w['cond']}}  {'Score':>{col_w['score']}}"
    )
    rows: list[str] = []
    ...
    for tid, labels_data in tmpl.items():
        ...
        for label, scores_data in labels_data.items():
            ...
            if isinstance(total, int):
                agg = cond_agg.setdefault(condition, {"score": 0, "max": 0, "count": 0})
                agg["score"] += total
                agg["max"] += max_pts
                agg["count"] += 1

            score_str = f"{total}/{max_pts}"
            rows.append(
                f"{tid:<{col_w['task']}}  {task['function']:<{col_w['fn']}}  "
                f"{run:<{col_w['run']}}  {condition:<{col_w['cond']}}  {score_str:>{col_w['score']}}"
            )
    ...
    for cond, data in sorted(cond_agg.items()):
        ...
        print(f"  {cond:<10}  {data['score']:>4} / {data['max']:<4}  ({pct:.1f}%)  "
              f"[{data['count']} response{'s' if data['count'] != 1 else ''}]")
    print()
...
def main() -> None:
    ...
    parser.add_argument("--base-url", default=None, dest="base_url", help="Base URL passed to bench.py")
    parser.add_argument("--api-key", default=None, dest="api_key", help="API key passed to bench.py")
    parser.add_argument(
        "--condition",
    ...
    parser.add_argument("--cofe-graph-bin", default=None, metavar="PATH", help="cofe-graph binary path")
    parser.add_argument("--project-path", default=None, dest="project_path", metavar="PATH")
    ...
    parser.add_argument("--judge-base-url", default=None, metavar="URL", dest="judge_base_url",
                        help="Base URL for judge LLM (defaults to --base-url)")
    parser.add_argument("--judge-api-key", default=None, metavar="KEY", dest="judge_api_key",
    ...
    if args.run:
        ...
        if len(args.paths) != 1:
            parser.error("--summary requires exactly one PATH (the judge output directory)")
        ...
        out_dir = args.out or (
            Path("results") / f"judge_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
