...
impl CodebaseGraph {
    ...
    pub fn add_edge(&mut self, caller: &str, callee: &str, line: u32) {
        self.callees
            .entry(caller.to_string())
            .or_default()
            .push(CallEdge { name: callee.to_string(), line });
        
        self.callers
            .entry(callee.to_string())
            .or_default()
            .push(CallEdge { name: caller.to_string(), line });
    }
        ...
        for (caller, edges) in fg.callees {
            for edge in &edges {
                g.callers
                    ...
                    .or_default()
                    .push(CallEdge { name: caller.clone(), line: edge.line });
            }
